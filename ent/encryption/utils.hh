...
class noop_log_filter: public http_log_filter {
public:
...
struct redacted {
    const T& original;
    seastar::shared_ptr<http_log_filter> filter;
...
template <>
struct fmt::formatter<encryption::redacted_request_type> : fmt::formatter<std::string_view> {
    auto format(const encryption::redacted_request_type&, fmt::format_context& ctx) const -> decltype(ctx.out());
};

template <>
struct fmt::formatter<encryption::redacted_result_type> : fmt::formatter<std::string_view> {
    auto format(const encryption::redacted_result_type&, fmt::format_context& ctx) const -> decltype(ctx.out());
};
