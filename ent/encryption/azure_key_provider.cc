...
public:
    azure_key_provider(::shared_ptr<azure_host> azure_host, std::string name)
    ...
    future<std::tuple<key_ptr, opt_bytes>> key(const key_info& info, opt_bytes id) override {
        ...
        return _azure_host->get_or_create_key(info).then([](std::tuple<key_ptr, opt_bytes> k_id) {
...
shared_ptr<key_provider> azure_key_provider_factory::get_provider(encryption_context& ctxt, const options& map) {
    ...
    auto id = azure_host.value();

    auto provider = ctxt.get_cached_provider(id);

    if (!provider) {
        provider = ::make_shared<azure_key_provider>(host, *azure_host);
