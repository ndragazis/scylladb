...
future<> replicated_key_provider_factory::on_started(encryption_context& ctxt, ::replica::database& db, service::migration_manager& mm, service::storage_service& ss) {
    auto version = co_await ss.get_system_keyspace().get_replicated_key_provider_version();
    co_await ctxt.set_replicated_keys_version(version);
