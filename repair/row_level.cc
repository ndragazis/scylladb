...
private:
    ...
    // Check if the follower has the same sharding flavor (vnodes or tablets) as its master.
    ...
    bool same_flavor_as_master() const {
        return _explicit_dst_cpu_id == _is_tablet;
    }
    ...
    bool can_read_locally() const {
        ...
        if (!same_flavor_as_master()) {
            if (!_table_is_migrating) {
                on_internal_error(rlogger, format(
                        "repair_meta[{}]: master/follower sharding flavor mismatch "
                        "(master_in_tablet_flavor={}, follower_in_tablet_flavor={}) outside "
                        "vnodes-to-tablets migration for table {}.{} range {}",
                        _repair_meta_id, _explicit_dst_cpu_id, _is_tablet,
                        _schema->ks_name(), _schema->cf_name(), _range));
            }
            return false;
        }
