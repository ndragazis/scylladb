...
tablets is strongly recommended.
...
* All nodes must be running a ScyllaDB version that supports this feature.
...
* **Counters and LWTs are not supported**.
...
.. There is a small race here. It could happen that an upgrading node is
.. restarted between marking it for downgrade and checking the status.
.. But I think it's quite unlikely and pretty obvious what needs to be
.. done in this case (ie. restart the node again).
