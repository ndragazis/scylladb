...
private:
    ...
    future<bool> maybe_initialize() {
        ...
        if (_single_partition_read) {
            ...
            _context = data_consume_rows<DataConsumeRowsContext>(*_schema, _sst, _consumer, std::move(drr), last_end, std::nullopt);
        }
...
public:
    mx_crawling_sstable_mutation_reader(shared_sstable sst, schema_ptr schema,
        ...
        , _context(data_consume_rows<DataConsumeRowsContext>(*_schema, _sst, _consumer, std::nullopt))
        , _monitor(mon) {
        _monitor.on_read_started(_context->reader_position());
    }
public:
...
future<uint64_t> validate(
    ...
    auto context = data_consume_rows<data_consume_rows_context_m<validating_consumer>>(*schema, sstable, consumer, std::nullopt);
