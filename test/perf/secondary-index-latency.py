...
def hydrate_table(session, partitions: int, partition_size: int):
    ...
    for pk in range(partitions):
        for ck in range(partition_size):
            session.execute(insert_query, (pk, 1, ck, random.randint(0, 1000)))
...
def main():
    cluster = Cluster(["127.46.0.1"]) # Use the right IP here
    ...
    session.execute("CREATE TABLE IF NOT EXISTS ks.t1 (pk1 int, pk2 int, ck int, value int, PRIMARY KEY ((pk1, pk2), ck));")
    session.execute("CREATE INDEX IF NOT EXISTS ON ks.t1 (pk2);")

    hydrate_table(session, partitions=1000, partition_size=100)
    #hydrate_table(session, partitions=100_000, partition_size=1)
    #hydrate_table(session, partitions=30_000, partition_size=1)
