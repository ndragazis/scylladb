...
async def test_no_cleanup_when_unnecessary(manager: ManagerClient):
    ...
    await manager.server_add(config=config, property_file={"dc": "dc1", "rack": "rack1"})
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}") as ks:
        ...
        await manager.server_add(config=config, property_file={"dc": "dc1", "rack": "rack2"})
        await manager.server_add(config=config, property_file={"dc": "dc1", "rack": "rack3"})
        ...
        await manager.server_add(config=config, property_file={"dc": "dc1", "rack": "rack4"})
        servers = await manager.running_servers()
        logs = [await manager.server_open_log(srv.server_id) for srv in servers]
        marks = [await log.mark() for log in logs]
