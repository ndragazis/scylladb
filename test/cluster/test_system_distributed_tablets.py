...
async def verify_schema(cql, expected_replication: dict[str, list[str]], timeout: int = 0, retry_interval: int = 1) -> None:
    async def _check():
...
async def test_auto_rf(manager: ManagerClient):
    ...
    logger.info("Restart a node and verify that the fourth rack is added to the RF")
    await manager.server_restart(servers[0].server_id)
    await verify_schema(cql, {'dc1': ['r1', 'r3', 'r4'], 'dc2': ['r1']})
...
async def test_upgrades(manager: ManagerClient):
    ...
    await verify_schema(cql, {'dc1': ['r1']}, timeout=20)
...
async def test_concurrent_rack_additions(manager: ManagerClient):
    ...
    await verify_schema(cql, {'dc1': ['r1', 'r2', 'r3']}, timeout=30)