...
from test.cluster.util import reconnect_driver, create_new_test_keyspace, new_test_keyspace, get_replication
...
async def test_tablet_count_metric_per_shard(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}") as ks:
        ...
        for ks in tablet_keyspaces:
            tables[ks] = [row.table_name for row in list(await cql.run_async(f"SELECT table_name FROM system_schema.tables WHERE keyspace_name='{ks}'"))]
