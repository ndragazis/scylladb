...
async def test_replace_during_migration(manager: ManagerClient):
    ...
    logger.info(f"Starting a cluster with {num_nodes} nodes, {num_shards} shards, {tokens_per_node} tokens per node")
    cfg = {'tablet_load_stats_refresh_interval_in_seconds': 1, 'num_tokens': tokens_per_node}
