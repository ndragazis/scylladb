...
future<> verify_that_all_sstables_are_local(sharded<sstable_directory>& sstdir, std::optional<unsigned> expected_sstables) {
...
SEASTAR_TEST_CASE(sstable_directory_test_reshard_vnodes) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        with_sstable_directory(e.db(), "ks", "cf", sstables::sstable_state::upload, [&] (sharded<sstables::sstable_directory>& sstdir) {
            ...
            verify_that_all_sstables_are_local(sstdir, std::nullopt).get();
        });
    });
}
