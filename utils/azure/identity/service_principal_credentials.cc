...
static std::optional<sstring> filter_request(const sstring& body) {
...
future<sstring> service_principal_credentials::post(const sstring& body) {
    ...
    auto print_request = [&] (const http::request& req) {
        ...
        fmt::format_to(fmt::appender(buf), "{} https://{}{} HTTP/{}{}", type2str(op), host, path, req._version, linesep);
        for (auto& [k, v] : req._headers) {
            fmt::format_to(fmt::appender(buf), "{}: {}{}", k, v, linesep);
        }
        fmt::format_to(fmt::appender(buf), "{}{}", filter_request(body).value_or(body), linesep);
