...
class topology_coordinator : public endpoint_lifecycle_subscriber
    ...
    future<std::optional<group0_guard>> maybe_schedule_auto_rf_change(group0_guard guard) {
        ...
        if (!_feature_service.topology_global_request_queue) {
            // RACK_LIST_RF was introduced after TOPOLOGY_GLOBAL_REQUEST_QUEUE,
            // so if the former is enabled, the latter must be too.
            on_internal_error(rtlogger, "topology_global_request_queue feature should have been enabled");
        }
